...
IMG_ID=$(docker image ls | grep $TAGNAME | grep -v _python-only | grep -v _slim | head -1 | awk '{print $3}')
export IMG_ID
...
IMG_ID=$(docker image ls | grep ${TAGNAME}_slim | head -1 | awk '{print $3}')
export IMG_ID
...
IMG_ID=$(docker image ls | grep ${TAGNAME}_python-only | head -1 | awk '{print $3}')
export IMG_ID
